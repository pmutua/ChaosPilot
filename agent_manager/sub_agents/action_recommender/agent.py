...
class IntelligentActionRecommender:
    ...
    def __init__(self):
        self.automation_playbooks = {
            "database_connection_issues": {
                "triggers": ["connection timeout", "connection pool exhausted", "database error"],
                "automated_actions": [
                    {
            ...
            "security_incident": {
                "triggers": ["failed login attempts", "suspicious activity", "unauthorized access"],
                "automated_actions": [
                    {
    ...
    def analyze_incident_for_actions(self, planner_data: Dict) -> Dict[str, Any]:
        ...
        incident_type = planner_data.get('incident_summary', {}).get('incident_type', '')
        priority = planner_data.get('incident_summary', {}).get('priority', '')
        patterns = planner_data.get('incident_summary', {}).get('detailed_analysis', {}).get('patterns_found', [])
        
        # Find matching playbook
        playbook = self._find_matching_playbook(patterns, incident_type)
        
        # Generate action recommendations
        recommendations = self._generate_action_recommendations(playbook, priority, patterns)
        ...
        return {
            ...
            "success_probability": self._calculate_success_probability(prioritized_actions),
            "estimated_resolution_time": self._estimate_resolution_time(prioritized_actions),
    ...
    def _find_matching_playbook(self, patterns: List[Dict], incident_type: str) -> Dict:
        ...
        return best_match or self.automation_playbooks.get("performance_degradation", {})
    
    def _generate_action_recommendations(self, playbook: Dict, priority: str, patterns: List[Dict]) -> List[Dict]:
    ...
    def _generate_action_reasoning(self, action: Dict, patterns: List[Dict]) -> str:
        ...
        if 'restart' in action_name:
            ...
            return "Manual investigation required to identify root cause of complex issues"
    ...
    def _generate_contextual_actions(self, patterns: List[Dict], priority: str) -> List[Dict]:
    ...
    def _prioritize_actions(self, actions: List[Dict], incident_priority: str) -> List[Dict]:
    ...
    def _create_automation_context(self, actions: List[Dict]) -> Dict[str, Any]:
        ...
        automated_actions = [a for a in actions if a.get('automation_level') == ActionType.AUTOMATED]
        semi_automated = [a for a in actions if a.get('automation_level') == ActionType.SEMI_AUTOMATED]
